...
func (s *DockerSuite) TestRunWithAdditionalRegistry(c *check.C) {
	...
	d.getAndTestImageEntry(c, 1, reg.url+"/busybox", busyboxId)

	// try to run fully qualified image
	if out, err := d.Cmd("run", "-t", reg.url+"/busybox", "sh", "-c", "echo foo"); err != nil {
		c.Fatalf("failed to run %s/busybox image: %v, output: %s", reg.url, err, out)
	...
	if out, err := d.Cmd("run", "-t", reg.url+"/library/hello-world", "sh", "-c", "echo foo"); err == nil {
		c.Fatalf("running container from image %s/library/hello-world should have failed; output: %s", reg.url, out)
	}
	...
	if out, err := d.Cmd("tag", reg.url+"/busybox", reg.url+"/library/hello-world"); err != nil {
		c.Fatalf("failed to tag image %s: error %v, output %q", reg.url+"/busybox", err, out)
	}
