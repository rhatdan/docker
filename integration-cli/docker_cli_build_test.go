...
func (s *DockerSuite) TestBuildWithAdditionalRegistry(c *check.C) {
  ...
  `, reg.url, name), true)
	...
	d.getAndTestImageEntry(c, 3, reg.url+"/library/hello-world", busyboxId)
...
func doTestBuildWithPublicRegistryBlocked(c *check.C, name string, daemonArgs []string) {
	allBlocked := false
  ...
  `, name), true)
	...
	if out, err := d.Cmd("push", reg.url+"/library/busybox"); !allBlocked && err != nil {
		c.Fatalf("failed to push image %s: error %v, output %q", reg.url+"/library/busybox", err, out)
...
func (s *DockerSuite) TestBuildWithPublicRegistryBlocked(c *check.C) {
	for _, blockedRegistry := range []string{"public", "docker.io"} {
		doTestBuildWithPublicRegistryBlocked(c, "testbuildpublicregistryblocked", []string{"--block-registry=" + blockedRegistry})
	}
}

func (s *DockerSuite) TestBuildWithAllRegistriesBlocked(c *check.C) {
	doTestBuildWithPublicRegistryBlocked(c, "testbuildwithallregistriesblocked", []string{"--block-registry=all"})
}
