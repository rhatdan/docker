...
func setupNewMountNamespace(rootfs, console string, readonly bool, mountLabel string) error {
...
func mountSystem(rootfs string, mountLabel string) error {
	for _, m := range []struct {
		...
		{source: "shm", path: filepath.Join(rootfs, "dev", "shm"), device: "tmpfs", flags: defaultMountFlags, data: label.FormatMountLabel("mode=1755", mountLabel)},
