...
func (s *TagStore) Pull(image string, tag string, imagePullConfig *ImagePullConfig, eng *engine.Engine) error {
	...
	doPull := func(image string) error {
		err := s.pullFromRegistry(image, tag, imagePullConfig, eng)
		return err
	}
...
func (s *TagStore) pullFromRegistry(image string, tag string, imagePullConfig *ImagePullConfig, eng *engine.Engine) error {
	var (
